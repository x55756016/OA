...
namespace SMT.FB.BLL
{
    ...
    public class BudgetAccountBLL : SubjectBLL
    {
        ...
        public bool GetCheckDeptBudgetSum(T_FB_DEPTBUDGETSUMMASTER entity, CheckStates statesNew)
        {
            try
            {
                ...
                { 
                    ...
                    if (master != null && master.SUMSETTINGSMASTERID != null)
                    {
                        //已生成二次汇总单
                        //查出此汇总单据对应的所有预算申请单id
                        var depSumMster = (from a in baseDal.GetTable<T_FB_DEPTBUDGETSUMMASTER>()
                                           where a.DEPTBUDGETSUMMASTERID == entity.DEPTBUDGETSUMMASTERID
                                           select a).FirstOrDefault();

                        if (!depSumMster.T_FB_DEPTBUDGETSUMDETAIL.IsLoaded) depSumMster.T_FB_DEPTBUDGETSUMDETAIL.Load();
                        string depMaterid = string.Empty;

                        //循环判断此汇总单里面的所有预算申请单是否已存在二次汇总单中，如果有一张不存在，就返回flase
                        foreach (var item in depSumMster.T_FB_DEPTBUDGETSUMDETAIL)
                        {
                            depMaterid = item.T_FB_DEPTBUDGETAPPLYMASTER.DEPTBUDGETAPPLYMASTERID;
                            //判断此汇总单里面的单据确认是否有二次汇总成功                      
                            var ent = from a in baseDal.GetTable<T_FB_DEPTBUDGETSUMMASTER>()
                                      join b in baseDal.GetTable<T_FB_DEPTBUDGETSUMDETAIL>()
                                      on a.DEPTBUDGETSUMMASTERID
                                      equals b.T_FB_DEPTBUDGETSUMMASTER.DEPTBUDGETSUMMASTERID
                                      join c in baseDal.GetTable<T_FB_DEPTBUDGETAPPLYMASTER>()
                                      on b.T_FB_DEPTBUDGETAPPLYMASTER.DEPTBUDGETAPPLYMASTERID
                                      equals c.DEPTBUDGETAPPLYMASTERID
                                      where a.DEPTBUDGETSUMMASTERID == master.DEPTBUDGETSUMMASTERID//二次汇总单
                                      && c.DEPTBUDGETAPPLYMASTERID == depMaterid
                                      select a;
                            //查询此汇总单号是否已汇总至二次汇总单中，不存在，需要生成。
                            if (ent.Count() == 0)
                            {
                                return false;
                            }
                    ...
                    {
                        //存在二次汇总设置，但没有生成二次汇总单，需要生成
                        Tracer.Debug("找到二次汇总设置，但没有二次汇总单，需要生成二次汇总单");
                        return false;
                    }
                }
                else
        ...
        public bool UpdateDeptBudgetSum(T_FB_DEPTBUDGETSUMMASTER entity, CheckStates statesNew)
        {
            try
            {
                Tracer.Debug("开始更新预算汇总单据,公司名："+entity.OWNERCOMPANYNAME+" 单号："+entity.DEPTBUDGETSUMMASTERCODE);
            ...
            {
                Tracer.Debug("更新预算汇总单失败：" + ex.ToString());
                return false;
            }
            ...
            try
            {
                ...
                var accountUpdates = from account in accountData
                                     ...
                                     equals
                                      ...
                                      join subject in this.GetTable<T_FB_SUBJECT>() on subjectCom.T_FB_SUBJECT.SUBJECTID equals subject.SUBJECTID
                                     select new { BudgetAccount = account, ControlType = subjectCom.CONTROLTYPE, SubjectID = account.T_FB_SUBJECT.SUBJECTID
                                     ,companyName=subjectCom.OWNERCOMPANYNAME,subjectName=subject.SUBJECTNAME};
                ...
                foreach (var accountUpdate in localaccountUpdates)
                {
                    ...
                    WriteLog("开始预算结算：公司名："+accountUpdate.companyName
                        +"科目id:" + accountUpdate.SubjectID 
                        +"科目名："+accountUpdate.subjectName
                        + "预算控制类型（0 不能跨月使用:1 :可以跨月使用; --无用的：2 不能跨月使用:  ; 3: 无限制 ; 4: 殊年结）："
                        + accountUpdate.ControlType.Value);
                    switch (cType)
                    {
                        case ControlType.CanNotcrossMonth: // 不跨月0
                            Tracer.Debug("结果为不能跨月，结算结果account.USABLEMONEY = 0");
                            account.USABLEMONEY = 0;
                            // 如果是跨年了
                            ...
                            break;
                        case ControlType.LimitMonth: // 不跨月2
                            account.USABLEMONEY = 0;
                            break;
                        case ControlType.LimitYear:  // 可以跨月1
                            // 如果是跨年了
                            if (System.DateTime.Now.Year > account.BUDGETYEAR)
                            {
                                account.USABLEMONEY = 0;
            ...
            try
            {
                ...
                try
                {
                    if (entityType == typeof(T_FB_PERSONMONEYASSIGNMASTER).Name && checkStates == CheckStates.Approved)
                    {
                        ...
                        //SMT.SaaS.BLLCommonServices.FBDailyManageWS.DailyManagementServicesClient ds 
                        //    = new SMT.SaaS.BLLCommonServices.FBDailyManageWS.DailyManagementServicesClient();
                        //ds.PersonmoneyAssignSendTask(orderID);


                        //工作计划活动经费接口，发布后一定要工作计划那边返回成功，否则正式环境活动经费会下拨失败。
                        try
                        {
                            T_FB_PERSONMONEYASSIGNMASTER master = entity as T_FB_PERSONMONEYASSIGNMASTER;

                            SMT.SaaS.BLLCommonServices.WPServicesWS.WPServicesClient client = new
                                SMT.SaaS.BLLCommonServices.WPServicesWS.WPServicesClient();
                            SMT.SaaS.BLLCommonServices.FBDailyManageWS.DailyManagementServicesClient FBAclient
                        ...
                        catch (Exception ex)
                        {
                            ...
                            strMsg = ex.ToString();
                            Tracer.Debug("调用工作计划相关接口异常：" + ex.ToString());
                        }
        ...
        public bool CheckAuditOrder(EntityObject entity, CheckStates oldStates, CheckStates newStates, ref string strMsg)
        {
            ...
            else if (entityType == typeof(T_FB_PERSONMONEYASSIGNMASTER))
            {
                try
                {
                    //工作计划活动经费接口，发布后一定要工作计划那边返回成功，否则正式环境活动经费会下拨失败。
                ...
                catch (Exception ex)
                {
                    result = false;
                    strMsg = ex.ToString();
                    Tracer.Debug("提交审核调用工作计划相关接口异常："+ex.ToString());
                }
