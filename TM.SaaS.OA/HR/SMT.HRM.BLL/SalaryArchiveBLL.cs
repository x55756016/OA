    ...
    public class SalaryArchiveBLL : BaseBll<T_HR_SALARYARCHIVE>, ILookupEntity, IOperate
    {
        ...
        public int SalaryArchiveDelete(string[] IDs)
        {
            ...
            try
            {
                ...
                List<T_HR_SALARYARCHIVE> dels = new List<T_HR_SALARYARCHIVE>();
        ...
        public T_HR_SALARYARCHIVE GetSalaryArchiveApprovedByEmployeeID(string employeeID, int year, int month, string companyid)
        {
            ...
            if (ents.Count() > 0)
            {
                var salaryList = ents.ToList();
                var list = salaryList.OrderByDescending(s => s.CREATEDATE).ThenByDescending(s => s.OTHERSUBJOIN).ThenByDescending(p => p.OTHERADDDEDUCT);
                foreach (T_HR_SALARYARCHIVE item in list)
                {
        ...
        public void CreateEmployeeSalaryArchive(T_HR_EMPLOYEE employee, T_HR_SALARYARCHIVE archive, int generateType, bool createType)
        {
            ...
            try
            {
                ...
                if (isForHuNanHangXingSalary == "true")
                {
                    try
                    {
                        //获取航信1-新技能工资和航信2-年工工资(b14bb5c0-1cd5-4682-8d09-62bdf56b5d1c)
                        var q = from ent in dal.GetObjects<T_HR_SALARYARCHIVEITEM>()
                                ...
                                && (ent.SALARYITEMID == "7955a9ae-b985-43c4-b42d-179e4e76d8ef" 
                                || ent.SALARYITEMID == "b14bb5c0-1cd5-4682-8d09-62bdf56b5d1c")                                
                                select ent;
                        //湖南航信特殊薪资项目
                        if (q.Count() > 0)
                        {
                            foreach (var archiveitem in q)
                            {
                                if (archiveitem.SALARYITEMID == "7955a9ae-b985-43c4-b42d-179e4e76d8ef")
                                {
                                ...
                                //年工工资
                                if (archiveitem.SALARYITEMID == "b14bb5c0-1cd5-4682-8d09-62bdf56b5d1c")
                                {
                                    string strsql = archiveitem.CALCULATEFORMULACODE + " where t_hr_employeeentry.employeeid='" + archive.EMPLOYEEID + "'";
                                    var value = dal.ExecuteCustomerSql(strsql);
                                    Tracer.Debug("湖南航信年工工资额:" + strsql + " 得到的结果：" + value.ToString());
                                    archiveitem.SUM = string.IsNullOrEmpty(value.ToString()) ? string.Empty : AES.AESEncrypt(value.ToString());
                                    dal.Update(archiveitem);
                                }
                            }
                    ...
                    {
                        Tracer.Debug("获取航信的新技能工资和年工工资出错:" + ex.ToString());
                    }
                }
        ...
        public void AddArchiveItems(T_HR_SALARYARCHIVE archive)
        {
           ...
           new System.Data.EntityKey(qualifiedEntitySetName + "T_HR_SALARYARCHIVE", "SALARYARCHIVEID", archive.SALARYARCHIVEID);
                ...
                {
                    if (item.T_HR_SALARYITEM.SALARYITEMID == "" || item.T_HR_SALARYITEM.SALARYITEMID == "")
                    {
