...
namespace SMT.HRM.BLL
{
    public class LeftOfficeConfirmBLL : BaseBll<T_HR_LEFTOFFICECONFIRM>, IOperate
    {
        ...
        public T_HR_LEFTOFFICECONFIRM getEditEmployeeLeftConfirmByEmpid(string employeeId)
        {
            var ent = from e in dal.GetObjects<T_HR_LEFTOFFICECONFIRM>()
                      where e.CHECKSTATE == "0"
        ...
        {
            return dal.GetObjects<T_HR_LEFTOFFICECONFIRM>().Include("T_HR_LEFTOFFICE").Include("T_HR_LEFTOFFICE.T_HR_EMPLOYEEPOST").FirstOrDefault(s => s.CONFIRMID == id);
        }
