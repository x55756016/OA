    ...
    public partial class AbnormRecordBLL : BaseBll<T_HR_EMPLOYEEABNORMRECORD>
    {
            ...
            string strAttStartTime, string strAttCardStartTime, string strAttCardEndTime, ref bool bCheck, ref StringBuilder strTemp, ref string strAbnormCategory)
        {
            ...
            try
            {
                ...
                T_HR_EMPLOYEEENTRY EmployeeEntry = entry.GetEmployeeEntryByEmployeeIDForAbnorm(entAttRd.EMPLOYEEID);
                //入职时间终审通过且更新时间比
                ...
                //    return;
                //}
                Tracer.Debug("异常签卡计算时间-dtAttStart:" + dtAttStart.ToString());
                Tracer.Debug("异常签卡员工入职时间-dtAttStart:" + EmployeeEntry.ENTRYDATE.Value.ToString());
                //改成入职时间的第2天和dtAttStart比较，集团需求
                if ( EmployeeEntry.ENTRYDATE.Value.AddDays(1) > dtAttStart )
                {
                    Tracer.Debug("检查异常，添加员工入职逻辑退出异常检查，员工ID:" + entAttRd.EMPLOYEEID);
                    return;
                }                
            }
            catch (Exception ex)
            ...
            string strAttOffCardStartTime, string strAttOffCardEndTime, ref bool bCheck, ref StringBuilder strTemp, ref string strAbnormCategory)
        {
            ...
            try
            {
                ...
                //    return;
                //}
                Tracer.Debug("异常签卡计算时间-dtAttEnd:" + dtAttEnd.ToString());
                Tracer.Debug("异常签卡员工入职时间-dtAttEnd:" + EmployeeEntry.ENTRYDATE.Value.ToString());
                //改成入职时间的第2天和dtAttStart比较，集团需求
                if (EmployeeEntry.ENTRYDATE.Value.AddDays(1) > dtAttEnd)
                {
                    Tracer.Debug("检查异常，添加员工入职逻辑退出异常检查，员工ID:" + entAttRd.EMPLOYEEID);
                    return;
                } 
            ...
            else
            {
                ...
                {   //如果当天正常，删除相关的异常考勤，签卡记录，待办任务
                    ...
                    if (item.T_HR_ATTENDANCERECORD.ATTENDANCESTATE == (Convert.ToInt32(Common.AttendanceState.Regular) + 1).ToString())
                    {
                        ...
                        //删除异常考勤
                       int i= dal.Delete(item);
                        if(i>0)
                        {
                            Tracer.Debug("删除错误的考勤异常成功，异常日期：" + item.ABNORMALDATE + "考勤状态：" + item.T_HR_ATTENDANCERECORD.ATTENDANCESTATE
                                                   + " 异常签卡状态：" + item.SINGINSTATE
                                                   + " 删除记录数"+i);
                        }
                    }
