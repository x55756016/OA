    ...
    public class SalaryRecordBatchBLL : BaseBll<T_HR_SALARYRECORDBATCH>, IOperate
    {
                ...
                foreach (var entItem in ents)
                {
                    ...
                    foreach (var detail in salaryItemsList)
                    {
                        ...
                        if (dictSum.ContainsKey("SALARYITEM" + n))
                        {
                            try
                            {
                                SumValue = Convert.ToDecimal(dictSum["SALARYITEM" + n]);
                                SumValue += string.IsNullOrEmpty(detail.SUM) ? 0 : Convert.ToDecimal(AES.AESDecrypt(detail.SUM));
                                dictSum["SALARYITEM" + n] = SumValue.ToString();
                            }
                            catch (Exception ex)
                            {
                                Tracer.Debug("获取薪资批量审核数据 计算报错：" + ex.ToString());
                            }
                        }
                        else
                        {
        ...
        public bool GetCompyanSalaryHasApproved(string companyid)
        {
            ...
            try
            {
                ...
                var ents = from ent in dal.GetObjects<T_HR_SALARYRECORDBATCH>()
                           where ent.OWNERCOMPANYID == companyid
                           && ent.CHECKSTATE =="2"
                           && ent.BALANCEYEAR == year
                           && ent.BALANCEMONTH == month
                           select ent;
                if (ents.Count() > 0)
                {
                    isReturn = true;
            ...
            {
                SMT.Foundation.Log.Tracer.Debug("GetCompyanSalaryHasApproved-出现错误:" + ex.Message);
            }
            return isReturn;
        }
