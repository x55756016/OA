...
namespace SMT.HRM.BLL
{
    public class OutApplyBLL : BaseBll<T_HR_OUTAPPLYRECORD>, IOperate
    {
        ...
        public IQueryable<T_HR_OUTAPPLYRECORD> EmployeeOutApplyRecordPaging(int pageIndex, int pageSize, string sort, string filterString, List<object> paras, ref int pageCount, string strCheckState, string strOwnerID)
        {
            ...
            if (!string.IsNullOrEmpty(filterString))
            {
                ents = ents.Where(filterString, paras.ToArray()).ToList().OrderByDescending(c=>c.STARTDATE).AsQueryable();
            }
        ...
        public string OutApplySetValue(string msg, T_HR_OUTAPPLYRECORD entity)
        {
            ...
            decimal totolHours = 0;
            string strMsg = CalculateOverTimeHours(entity.EMPLOYEEID, entity.STARTDATE.Value, entity.ENDDATE.Value, ref dTotalHours, ref totolHours);
            Tracer.Debug("外出时长："+totolHours.ToString());
            entity.OUTAPLLYTIMES = totolHours.ToString();
            return strMsg;
        }
        ...
        public int UpdateOutApply(T_HR_OUTAPPLYRECORD entity,string falg)
        {
           return UpdateOutApply(entity);
        }
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (entOTRd.CHECKSTATE == Convert.ToInt32(CheckStates.Approved).ToString())
                    {
                        ...
                        if (!string.IsNullOrEmpty(isForHuNanHangXingSalary))
                        {
        ...
        /// <returns></returns>
        public string CalculateOverTimeHours(string strEmployeeId, DateTime dtOTStart, DateTime dtOTEnd, ref string dOverTimeHours, ref decimal totolHours)
        {
            ...
            totolHours =dTotalOverTimeHours;
            if (ts.Minutes > 0)
            {
                decimal hours=ts.Milliseconds / 60;
                totolHours = totolHours + Math.Round(hours, 1);
            }
