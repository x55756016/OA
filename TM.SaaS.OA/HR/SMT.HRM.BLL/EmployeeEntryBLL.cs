...
namespace SMT.HRM.BLL
{
    public class EmployeeEntryBLL : BaseBll<T_HR_EMPLOYEEENTRY>, IOperate
    {
        ...
        public string EmployeeEntryAdd(T_HR_EMPLOYEE employee, T_HR_EMPLOYEEENTRY employeeEntry, T_HR_EMPLOYEEPOST employeePost)
        {
            ...
            try
            {
                SMT.Foundation.Log.Tracer.Debug(System.DateTime.Now.ToString() + "开始调用 EmployeeEntryAdd:" + employee.REMARK + " " + employee.EMAIL);
                ...
                if (tempEnt.Count() > 0)
                {
                    try
                    {
                        SMT.Foundation.Log.Tracer.Debug(System.DateTime.Now.ToString() + "EmployeeEntryAdd:找到相同身份证的员工" + tempEnt.FirstOrDefault().IDNUMBER);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        SMT.Foundation.Log.Tracer.Debug(System.DateTime.Now.ToString() + " EmployeeEntryAdd更新失败:" + ex.Message);
                ...
                {
                    SMT.Foundation.Log.Tracer.Debug(System.DateTime.Now.ToString() + " EmployeeEntry 开始保存T_HR_EMPLOYEE:");
                    ...
                    SMT.Foundation.Log.Tracer.Debug(System.DateTime.Now.ToString() + " EmployeeEntry 保存成功T_HR_EMPLOYEE:");
                    //  dal.SaveContextChanges();
                }
        ...
        /// <param name="employeeID"></param>
        /// <returns></returns>
        public T_HR_EMPLOYEEENTRY GetEmployeeEntryByEmployeeIDForAbnorm(string employeeID)
        {
            var q = from ent in dal.GetObjects().Include("T_HR_EMPLOYEE")
                    where ent.T_HR_EMPLOYEE.EMPLOYEEID == employeeID && (ent.CHECKSTATE == "2" || ent.CHECKSTATE =="1")
                    orderby ent.ENTRYDATE descending
                    select ent;
            //时间排序尚未实现故需先tolist
            var q2 = q.ToList().OrderByDescending(c => c.ENTRYDATE);
            ...
            try
            {
                ...
                if (tmp != null)
                {
                    ...
                    if (CheckState == Convert.ToInt32(CheckStates.Approved).ToString())
                    {
                        ...
                        {
                            SMT.Foundation.Log.Tracer.Debug("员工入职开始调工作计划接口AddEmployeePlan异常" + ex.Message.ToString());
                        }
                        #endregion
