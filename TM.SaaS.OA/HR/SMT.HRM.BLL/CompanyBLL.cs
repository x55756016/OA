...
namespace SMT.HRM.BLL
{
    public class CompanyBLL : BaseBll<T_HR_COMPANY>, ILookupEntity, IOperate
    {
        ...
        /// <param name="employeeID"></param>
        private void SetCompanyFilterByApproval(ref string filterString, ref List<object> queryParas, string employeeID)
        {
            if (string.IsNullOrEmpty(filterString))
            {
                return;
        ...
        /// <summary>
        /// 给事项审批过滤公司使用，去掉公司权限过滤
        /// </summary>
        /// <param name="userID"></param>
        /// <param name="perm"></param>
        /// <param name="entity"></param>
        /// <returns></returns>
        public IQueryable<V_COMPANY> GetCompanyViewByApproval(string userID, string perm, string entity)
        {
            #region 根据权限过滤数据
            if (string.IsNullOrEmpty(perm))
            {
            ...
            if (!string.IsNullOrEmpty(userID))
            {
                SetOrganizationFilterForApprovalCompany(ref filterString, ref paras, userID, entity, perm);
                SetCompanyFilterByApproval(ref filterString, ref paras, userID);
            }
            #endregion
            IQueryable<T_HR_COMPANY> ents = from c in dal.GetObjects()
                                            select c;
        ...
        /// <param name="employeeID"></param>
        /// <returns></returns>
        public List<T_HR_COMPANY> GetAllCompanysByEmployeeID(string employeeID)
        {
            List<T_HR_COMPANY> listComs = new List<T_HR_COMPANY>();
            try
            {
                ...
                foreach (var q in ents)
                {
                    ...
                    c.COMPANYID = q.COMPANYID;
                    c.CNAME = q.CNAME;
                     c.BRIEFNAME = q.BRIEFNAME == null ? q.CNAME : q.BRIEFNAME;
                    c.FATHERID = q.FATHERID;
                    c.FATHERTYPE = q.FATHERTYPE;
                    listComs.Add(c);
                }
            ...
            {
                Tracer.Debug("CompanyBll-GetAllCompanysByEmployeeID，员工ID:" + employeeID +"。错误信息：" + ex.ToString());
            }
            return listComs;
        }
