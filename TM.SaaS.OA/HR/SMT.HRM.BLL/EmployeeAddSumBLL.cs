...
namespace SMT.HRM.BLL
{
    public class EmployeeAddSumBLL : BaseBll<T_HR_EMPLOYEEADDSUM>, IOperate
    {
        ...
        public IQueryable<T_HR_EMPLOYEEADDSUM> QueryWithPaging(int pageIndex, int pageSize, string sort, string filterString, IList<object> paras, ref int pageCount, DateTime starttime, DateTime endtime, string userID, string CheckState, int orgtype, string orgid)
        {
            ...
            SMT.Foundation.Log.Tracer.Debug("员工加扣款耗时QueryWithPaging：" + (dt2 - dt).Milliseconds.ToString());
            ents = Utility.Pager<T_HR_EMPLOYEEADDSUM>(ents, pageIndex, pageSize, ref pageCount);
        ...
        {
            IQueryable<T_HR_EMPLOYEEADDSUM> ents = dal.GetObjects().Include("T_HR_EMPLOYEEADDSUMBATCH");
            //先过滤出数据
            //ents = ents.Where(s=>s.OWNERCOMPANYID == orgid || s.OWNERDEPARTMENTID ==orgid || s.OWNERPOSTID ==orgid);
            DateTime dt = DateTime.Now;       
            switch (orgtype)
            {
                case 0:
                    //ents = from a in ents //dal.GetObjects<T_HR_EMPLOYEEADDSUM>().Include("T_HR_EMPLOYEEADDSUMBATCH")
            ...
            DateTime dt2 = DateTime.Now;
            SMT.Foundation.Log.Tracer.Debug("员工加扣款耗时111："+ (dt2-dt).Milliseconds.ToString());
