...
namespace SMT.HRM.BLL
{
    public class EmployeePostChangeBLL : BaseBll<T_HR_EMPLOYEEPOSTCHANGE>, IOperate
    {
        ...
        public int UpdateCheckState(string strEntityName, string EntityKeyName, string EntityKeyValue, string CheckState)
        {
            ...
            logMsg.AppendLine("员工岗位异动UpdateCheckState开始");
            logMsg.AppendLine("员工岗位异动审核：strEntityName=" + strEntityName + " EntityKeyName=" + EntityKeyName + " EntityKeyValue=" + EntityKeyValue + " CheckState=" + CheckState);
            try
            {
                int i = 0;
                ...
                if (CheckState == Convert.ToInt32(CheckStates.Approved).ToString() || CheckState == Convert.ToInt32(CheckStates.Approving).ToString())
                {
                    ...
                    {
                        logMsg.AppendLine("FormID:" + EntityKeyValue + " ---岗位id:" + ent.TOPOSTID + "生效状态" + post.GetPostById(ent.TOPOSTID).EDITSTATE);
                        return i;
                    }
                }
                #endregion
                ...
                if (tmp != null)
                {
                    ...
                    if (CheckState == Convert.ToInt32(CheckStates.Approved).ToString())
                    {
                        ...
                        logMsg.AppendLine("查询员工岗位表EMPLOYEEID=" + employeeID + ",FROMPOSTID=" + employeePostChange.FROMPOSTID + "EDITSTATE=1的记录");
                        
                        if (employeePost != null)
                        {
                            logMsg.AppendLine("查询出来的员工岗位表EMPLOYEEPOSTID=" + employeePost.EMPLOYEEPOSTID);
                            if (employeePostChange.ISAGENCY == "3") //主兼岗位互换
                            {
                                logMsg.AppendLine("employeePostChange.ISAGENCY=3 主兼岗位互换");
                                #region 主兼职岗位异动
                                //原来岗位
                            ...
                            {
                                logMsg.AppendLine("employeePost.ISAGENCY == " + employeePost.ISAGENCY + " || employeePostChange.ISAGENCY == " + employeePostChange.ISAGENCY);
                                #region 主岗位异动
                                //更具员工ID查询员工基本信息表
                                ...
                                var eOldPost = (from ep in dal.GetObjects<T_HR_EMPLOYEEPOST>()
                                                ...
                                                select ep).FirstOrDefault();
                                logMsg.AppendLine("查询旧职位T_HR_EMPLOYEEPOST where EMPLOYEEID=" + employeeID + " ep.ISAGENCY ==" + 0 + " && ep.EDITSTATE == " + 1);

                                var epost = (from en in dal.GetObjects<T_HR_EMPLOYEEPOST>()
                                             where en.EMPLOYEEPOSTID == employeePostChange.EMPLOYEEPOSTID
                                             select en).FirstOrDefault();
                                logMsg.AppendLine("查询T_HR_EMPLOYEEPOST where EMPLOYEEPOSTID=" + employeePostChange.EMPLOYEEPOSTID);
                                if (epost != null)
                                {
                                    logMsg.AppendLine("更新员工岗位表");
                                ...
                                //主岗位异动
                                if (employeePostChange.ISAGENCY != "1" && employeePostChange.FROMPOSTID != employeePostChange.TOPOSTID)
                                {
                                    MailToManager(employee.EMPLOYEECNAME, employee.EMPLOYEEID,
                                    employeePostChange.FROMDEPARTMENTID, employeePostChange.FROMPOSTID,
                                    employeePostChange.FROMCOMPANYID);
                                }
                            }
                            else
                            {
                        ...
                        #region 调用即时通讯的接口
                        AddIMInstantMessageForPostChange(employeePostChange);
                        #endregion

                        #region 员工异动工作计划接口
                        try
                        {
                            //SMT.Foundation.Log.Tracer.Debug("员工异动开始调用工作计划接口:employeePostChange.postchangeid=" + employeePostChange.POSTCHANGEID + ",employeePostChange.ISAGENCY=" + employeePostChange.ISAGENCY);
                            //if (employeePostChange.ISAGENCY == "0" || employeePostChange.ISAGENCY == "3")
                            //{
                            //    string StrEmployeePostID = employeePostChange.EMPLOYEEPOSTID;
                            ...
                            //    vemployee.OWNERID = employeePostChange.OWNERID;
                            //    vemployee.OWNERNAME = employeePostChange.EMPLOYEENAME;
                            //    vemployee.OWNERPOSTID = entsEmployeePosts.FirstOrDefault().POSTID;
                            //    vemployee.OWNERPOSTNAME = entsEmployeePosts.FirstOrDefault().PostName;
                            //    vemployee.OWNERDEPARTMENTID = entsEmployeePosts.FirstOrDefault().DepartmentID;
                            //    vemployee.OWNERDEPARTMENTNAME = entsEmployeePosts.FirstOrDefault().DepartmentName;
                            //    vemployee.OWNERCOMPANYID = entsEmployeePosts.FirstOrDefault().CompanyID;
                            //    vemployee.OWNERCOMPANYNAME = entsEmployeePosts.FirstOrDefault().CompanyName;
                            //    client.EmployeePostChangeAuditPass(employeePostChange.FROMPOSTID, vemployee);
                            //}
                        }
                        catch (Exception ex)
                        {
                            SMT.Foundation.Log.Tracer.Debug(ex.Message.ToString());
                        }
                        #endregion
                    }
                ...
                logMsg.Append("员工岗位异动UpdateCheckState结束");
                SMT.Foundation.Log.Tracer.Debug(logMsg.ToString());
                return i;
            }
            catch (Exception e)
            {
                logMsg.AppendLine("异常："+e.Message.ToString());
                SMT.Foundation.Log.Tracer.Debug(logMsg.ToString());
        ...
        public string IfDepartmentHaveEmployee(string departmentId,string postId,string employeeId)
        {
            ...
            bool employeePostAny = false;
            List<T_HR_EMPLOYEEPOST> employeePostList =
                        dal.GetObjects<T_HR_EMPLOYEEPOST>().Include("T_HR_POST").Include("T_HR_EMPLOYEE")
                            .Where(
                                t => t.T_HR_POST.POSTID == postId && t.T_HR_EMPLOYEE.EMPLOYEEID != employeeId &&
                                    t.CHECKSTATE == "2" && t.EDITSTATE == "1")
                            .ToList();
            if (employeePostList != null && !employeePostList.Any())
            {
                employeePostAny = true;
            }

            #region 判断部门负责人
            
            T_HR_DEPARTMENT department =
                    dal.GetObjects<T_HR_DEPARTMENT>()
                    .Include("T_HR_DEPARTMENTDICTIONARY")
                    .Where(t => t.DEPARTMENTID == departmentId).FirstOrDefault();
            
            if (department != null)
            {
                if (employeePostAny)
                {
                    var deptList = dal.GetObjects<T_HR_DEPARTMENT>()
                        .Include("T_HR_DEPARTMENTDICTIONARY")
                        .Where(t => t.DEPARTMENTBOSSHEAD == postId && t.CHECKSTATE=="2" && t.EDITSTATE=="1").ToList();

                    string deptStr = string.Empty;
                    if (deptList != null && deptList.Any())
                    {
                        condition1 = true;
                        foreach (T_HR_DEPARTMENT dept in deptList)
                        {
                            deptStr += dept.T_HR_DEPARTMENTDICTIONARY.DEPARTMENTNAME + ",";
                        }
                        sbMsg.Append(string.Format("此部门下的岗位没有人员，请修改{0}的部门负责人", deptStr.Trim(',')));
                    }
            ...
            if (employeePostAny)
            {
                List<T_HR_POST> postFatherList = dal.GetObjects<T_HR_POST>()
                    .Include("T_HR_POSTDICTIONARY")
                    .Where(t => t.FATHERPOSTID == postId && t.CHECKSTATE == "2" && t.EDITSTATE == "1").ToList();
                if (postFatherList != null && postFatherList.Any())
                {
                    string postStr = string.Empty;
                    foreach (T_HR_POST post in postFatherList)
                    {
                        postStr += post.T_HR_POSTDICTIONARY.POSTNAME + ",";
                    }
                    if (condition1)
                    {
                        sbMsg.Append(string.Format("以及{0}岗位的直接上级。", postStr.Trim(',')));
                    }
                    else
                    {
                        sbMsg.Append(string.Format("此部门下的岗位没有人员,请修改{0}岗位的直接上级。", postStr.Trim(',')));
                    }
                }
            }
            #endregion
        ...
        private void MailToManager(string employeeName,string employeeId,string departmentId,string postId,string ownerCompanyId)
        {
            try
            {
                SMT.Foundation.Log.Tracer.Debug("员工异动调用邮件通知管理员开始");
                
                T_HR_DEPARTMENT department =
                    dal.GetObjects<T_HR_DEPARTMENT>().Where(t => t.DEPARTMENTID == departmentId).FirstOrDefault();
                //如果部门负责人岗位是异动的岗位
                if (department != null && department.DEPARTMENTBOSSHEAD == postId)
                {
                    //判断这岗位下还有没有人。
                    List<T_HR_EMPLOYEEPOST> employeePostList =
                        dal.GetObjects<T_HR_EMPLOYEEPOST>().Include("T_HR_POST").Include("T_HR_EMPLOYEE")
                    ...
                    {
                        var postList =
                        dal.GetObjects<T_HR_POST>()
                            .Include("T_HR_POSTDICTIONARY")
                            .Include("T_HR_DEPARTMENT.T_HR_DEPARTMENTDICTIONARY")
                            .Where(t => t.OWNERDEPARTMENTID == departmentId && t.CHECKSTATE == "2" && t.EDITSTATE == "1")
                            .ToList();
                        string postName =
                            postList.Where(t => t.POSTID == postId).FirstOrDefault().T_HR_POSTDICTIONARY.POSTNAME;
                        string postStr = string.Empty;
                        string deptName = string.Empty;
                        if (postList != null && postList.Any())
                        {
                            deptName = postList[0].T_HR_DEPARTMENT.T_HR_DEPARTMENTDICTIONARY.DEPARTMENTNAME;
                            var exceptPost = postList.Where(t => t.POSTID != postId).ToList();
                            foreach (T_HR_POST post in exceptPost)
                            {
                                postStr += post.T_HR_POSTDICTIONARY.POSTNAME + ",";
                            }
                        }
                        ...
                        foreach (var sysManager in sysManagers)
                        {
                            ...
                            mParams.MailTitle = employeeName + "岗位异动";
                            mParams.MailContent = string.Format("因{0}岗位异动，此部门下的{1}岗位没有人员，会影响{2}的部门负责人及{3}岗位的直接上级，请修改正确。",
                                employeeName, postName, deptName, postStr.Trim(','));
                            var employee =
                                dal.GetObjects<T_HR_EMPLOYEE>()
                                    .Where(t => t.EMPLOYEEID == sysManager.EMPLOYEEID)
                                    .FirstOrDefault();
                            if (employee != null && !string.IsNullOrEmpty(employee.EMAIL))
                            {
                                mailStr += employee.EMAIL + ",";
                                mParams.ReceiveUserMail = employee.EMAIL;
                                mailList.Add(mParams);
                            }
                        }
                        MailParams[] mailArray = mailList.ToArray();
                        mailClient.SendMail(mailArray);
                        SMT.Foundation.Log.Tracer.Debug("员工" + employeeName + "异动，发邮件给" + mailStr.Trim(',') + "");
                    }
                    else
                    {
                        SMT.Foundation.Log.Tracer.Debug("该员工"+employeeName+"异动的岗位还有人，则不发邮件给管理员");
                    }
                }
                else
                {
                    SMT.Foundation.Log.Tracer.Debug("该员工" + employeeName + "异动的岗位(" + postId + ")不是部门负责人的岗位，则不发邮件给管理员");
                }
            }
            catch (Exception ex)
            {
                SMT.Foundation.Log.Tracer.Debug("员工异动调用邮件通知管理员错误：" + ex.Message.ToString());
            }
        ...
        private void AddIMInstantMessageForPostChange(T_HR_EMPLOYEEPOSTCHANGE EntObj)
        {
            ...
            try
            {
                ...
                else if (EntObj.ISAGENCY == "3")
                {
                    SMT.Foundation.Log.Tracer.Debug("主兼职岗位互换开始调用即时通讯接口");
                    IMCient.EmployeePostChange(StrUserID, StrFromPostID, StrToPostID);
                }
            }
            catch (Exception ex)
            {
        ...
        public bool EmployeeCanPostChange(string employeeId, string fromPostId)
        {
            try
            {
                //SMT.Foundation.Log.Tracer.Debug("员工异动开始调用工作计划接口");
                //SMT.SaaS.BLLCommonServices.WPServicesWS.WPServicesClient client = new SaaS.BLLCommonServices.WPServicesWS.WPServicesClient();
                //return client.EmployeeCanPostChange(employeeId, fromPostId);
                return true;
            }
            catch (Exception ex)
            {
                SMT.Foundation.Log.Tracer.Debug("员工异动开始调用工作计划接口:" + ex.Message.ToString());
                return true;
            }
        }
    }
