...
namespace SMT.HRM.BLL.Permission
{
    public class SysPermissionBLL:BaseBll<T_SYS_PERMISSION>
    {
        ...
        public string SysPermissionUpdate(T_SYS_PERMISSION entity)
        {
            ...
            try
            {
                ...
                if (ents.Count() > 0)
                {
                    ...
                    //    new System.Data.EntityKey("TM_SaaS_OA_EFModelContext.T_SYS_ENTITYMENU", "ROLEID", entity.T_SYS_ENTITYMENUReference.EntityKey.EntityKeyValues[0].Value.ToString());
        ...
        public List<T_SYS_USER> GetManagers(List<string> companyIDs)
        {
            ...
            {
                var ents = from ent in dal.GetObjects<T_SYS_USER>()
                           where ent.STATE == "1" && ent.ISMANGER == 1
                           && companyIDs.Contains(ent.OWNERCOMPANYID)
                           select ent;
                if (ents.Count() > 0)
                {
            ...
            {
                SMT.Foundation.Log.Tracer.Debug("获取某公司的管理员出错" + ex.ToString());
            }
            return listUsers;
        }
