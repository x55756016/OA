...
namespace SMT.HRM.Services
{
    public partial class OrganizationService
    {
        ...
        /// <returns></returns>
        [OperationContract]
        public List<V_EMPLOYEEVIEW> GetEmployeeViewsPagingForMVCNoFilter(int pageIndex, int pageSize, string sort, string filterString, string[] paras, ref int pageCount, string sType, string sValue, string userID, ref int recordCount)
        {
            using (EmployeeBLL bll = new EmployeeBLL())
            {
                List<V_EMPLOYEEVIEW> q = bll.GetEmployeeViewsPagingForMVCNoFilter(pageIndex, pageSize, sort, filterString, paras, ref  pageCount, sType, sValue, userID, ref recordCount);
                return q;
            }
        }
        ...
        public List<V_EmployeeEntryInfo> ImportEmployeeEntry(UploadFileModel uploadFile, string companyID, Dictionary<string, string> empInfoDic)
        {
            using (EmployeeEntryBLL bll = new EmployeeEntryBLL())
            {
                ...
                SaveFileReturnFilePath(uploadFile, out strPath);//获取文件路径
        ...
        public List<V_EMPLOYEEVIEW> GetEmployeeInfosByEmployeeIDs(List<string> employeeIDs)
        {
            using (EmployeeBLL bll = new EmployeeBLL())
            {
                return bll.GetEmployeeInfosByEmployeeIDs(employeeIDs);
            }
        }
        ...
        public bool EmployeeCanPostChange(string employeeId, string fromPostId)
        {
            using (EmployeePostChangeBLL bll = new EmployeePostChangeBLL())
            {
                return bll.EmployeeCanPostChange(employeeId, fromPostId);
            }
        }
    }
}
