...
namespace SMT.HRM.Services
{
    ...
    public class SalaryService
    {
        ...
        public T_HR_SYSTEMSETTING GetSystemSettingByCompanyId(string ownerCompanyId)
        {
            using (SalaryLoginBLL bll = new SalaryLoginBLL())
            {
                return bll.GetSystemSettingByCompanyId(ownerCompanyId);
            }
        }
        #endregion
        ...
        [OperationContract]
        public void AssignPersonMoneyNew(string strCheckState, string Companyid, string createUserID,ref string strMsg)
        {
            using (SalaryRecordBatchBLL bll = new SalaryRecordBatchBLL())
            {
                bll.AssignPersonMoney(strCheckState, Companyid, createUserID, ref strMsg);
            }
        }
        ...
        /// <returns></returns>
        [OperationContract]
        public bool GetCompyanSalaryHasApproved(string Companyid)
        {
            using (SalaryRecordBatchBLL bll = new SalaryRecordBatchBLL())
            {
                return bll.GetCompyanSalaryHasApproved(Companyid);
            }
        }
        ...
        /// 添加员工薪资结算岗位变更
        /// </summary>
        /// <param name="obj">薪资岗位变更实例</param>
        [OperationContract]
        public string EmployeeBalancePostADD(T_HR_EMPLOYEESALARYPOSTASIGN obj, List<T_HR_BALANCEPOSTDETAIL> details, string flag, ref string strResult)
        {
            using (EmployeeBalancePostBLL bll = new EmployeeBalancePostBLL())
            {
                return bll.BalancePostADD(obj, details, flag, ref strResult);
            }
        }
        ...
        /// <param name="flag">flag:1 第1次保存  2：第2次保存</param>
        /// <returns></returns>
        [OperationContract]
        ...
        /// 删除员工薪资结算岗位变更      
        /// </summary>
        /// <param name="IDs">薪资岗位变更ID</param>
        /// <returns></returns>
        [OperationContract]
        public bool EmployeeBalancePostDelete(string[] IDs)
        {
            using (EmployeeBalancePostBLL bll = new EmployeeBalancePostBLL())
            {
                int rslt = bll.BalancePostDelete(IDs);
                return (rslt > 0);
            }
        }
